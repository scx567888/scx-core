...
public class OrganizationAuthHandler implements AuthHandler {
    ...
    public Json infoUpdate(User newUserInfo) {
        ...
        if (userService.update(currentUser) != null) {
            Ansi.out().color("更新了自己的信息 用户名是 :" + currentUser.username).println();
            return Json.ok();
        } else {
            return Json.fail();
        }
    ...
    public Json logout() {
        ...
        Ansi.out().color("当前总登录用户数量 : " + ScxAuth.getAllLoginItem().size() + " 个").println();
    ...
    private Json authExceptionHandler(AuthException e) {
        if (e instanceof UnknownDeviceException) {
            ...
            Ansi.out().color("登录出错 : " + e.getMessage()).println();
    ...
    public Json login(String username, String password) {
        try {
            ...
            Ansi.out().color(loginUser.username + " 登录了 , 登录设备 [" + loginDevice + "] , 当前总登录用户数量 : " + ScxAuth.getAllLoginItem().size() + " 个").println();
