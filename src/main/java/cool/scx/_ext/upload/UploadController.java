...
public class UploadController {
     ...
     * @throws cool.scx.exception.HttpRequestException if any.
     */
    @ScxMapping(value = "/show/:fileId", method = {Method.GET, Method.HEAD})
    public BaseVo show(@FromPath String fileId,
    ...
    public Json upload(String fileName, Long fileSize, String fileMD5, Integer chunkLength, Integer nowChunkIndex, FileUpload fileData) {
        var uploadTempFile = UploadConfig.uploadFilePath().getPath() + "\\TEMP\\" + fileMD5 + "_" + fileName + "\\.scxTemp";
        var uploadConfigFile = new File(UploadConfig.uploadFilePath().getPath() + "\\TEMP\\" + fileMD5 + "_" + fileName + "\\.scxUpload");
        //先判断 文件是否已经上传过
        if (StringUtils.isNotEmpty(fileMD5)) {
            ...
            if (fileByMd5 != null) {
                File file = new File(UploadConfig.uploadFilePath() + "\\" + fileByMd5.filePath);
                if (file.exists()) {
        ...
        if (nowChunkIndex.equals(chunkLength)) {
            ...
            //获取文件真实的存储路径
            var fileStoragePath = UploadConfig.uploadFilePath().getPath() + "\\" + uploadFile.filePath;
    ...
    public Json deleteFile(String fileId) {
        ...
        if (needDeleteFile != null) {
            ...
            //没有被其他人引用过 可以删除物理文件
            if (count == 1) {
                var filePath = UploadConfig.uploadFilePath() + "\\" + needDeleteFile.filePath;
                var file = new File(filePath);
                if (file.exists()) {
                    boolean b = FileUtils.deleteFiles(Path.of(UploadConfig.uploadFilePath() + "\\" + needDeleteFile.filePath).getParent());
                    if (!b) {
                        return Json.fail();
                    }
                }
