...
public  abstract class UserService extends BaseService<User> {
    ...
    public User findByUsername(String username) {
        User user = ScxContext.GetImpl(User.class);
        var param = new Param<>(user);
    ...
    public HashSet<String> getPermStrByUser(User user) {
        ...
        if (user.level < 5) {
            ...
            roleService.getRoleListByUser(user).stream().filter(role -> StringUtils.isNotEmpty(role.perm)).map(role -> role.perm).forEach(permList::addAll);
            deptService.getDeptListByUser(user).stream().filter(dept -> StringUtils.isNotEmpty(dept.perm)).map(Dept -> Dept.perm).forEach(permList::addAll);
    ...
    public User updateUserAndDept(Long id, boolean b) {
        var user = ScxContext.GetImpl(User.class);
