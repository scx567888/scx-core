...
public class Scx {
    ...
    public Scx(File configPath) {
        ...
        uploadFilePath = getConfigValue("scx.upload-file-path", "/scxUploadFile/",
                ...
                f -> LogUtils.println("✘ 未检测到 scx.upload-file-path        \t -->\t 已采用默认值 : " + f, Color.RED),
                JsonNode::asText, a -> a);
        ...
        bodyLimit = getConfigValue("scx.body-limit", "16384KB",
                ...
                f -> LogUtils.println("✘ 未检测到 scx.body-limit             \t -->\t 已采用默认值 : " + FileUtils.longToDisplaySize(FileUtils.displaySizeToLong(f)), Color.RED),
                JsonNode::asText, a->a);
        ...
        if (needFixConfig.get()) {
            ...
            try {
                ...
                objectMapper.writerWithDefaultPrettyPrinter().writeValue(outputStream, config);
